/* Reef Response field sites — derived from "Reef Response on the Map" (Google Earth KML).
   Edit freely: `type` is either "facility" or "site", and drives the marker colour. */
var REEF_SITES = [
  {
    "name": "Center for Marine and Environmental Studies",
    "lat": 18.3401,
    "lng": -64.97655,
    "region": "St. Thomas — South Shore",
    "type": "facility"
  },
  {
    "name": "Brewers",
    "lat": 18.34459,
    "lng": -64.98359,
    "region": "St. Thomas — South Shore",
    "type": "site"
  },
  {
    "name": "Fortuna",
    "lat": 18.34504,
    "lng": -65.01548,
    "region": "St. Thomas — South Shore",
    "type": "site"
  },
  {
    "name": "Artificial Reef",
    "lat": 18.34474,
    "lng": -65.01525,
    "region": "St. Thomas — South Shore",
    "type": "facility"
  },
  {
    "name": "Perseverance",
    "lat": 18.34944,
    "lng": -64.99454,
    "region": "St. Thomas — South Shore",
    "type": "site"
  },
  {
    "name": "Range Cay",
    "lat": 18.33947,
    "lng": -64.97843,
    "region": "St. Thomas — South Shore",
    "type": "site"
  },
  {
    "name": "Flat Cay — Leeward",
    "lat": 18.31756,
    "lng": -64.99046,
    "region": "St. Thomas — South Shore",
    "type": "site"
  },
  {
    "name": "Flat Cay — Windward",
    "lat": 18.31715,
    "lng": -64.9887,
    "region": "St. Thomas — South Shore",
    "type": "site"
  },
  {
    "name": "South Capella",
    "lat": 18.26163,
    "lng": -64.87242,
    "region": "St. Thomas — South Shore",
    "type": "site"
  },
  {
    "name": "Rupert's Rock",
    "lat": 18.3272,
    "lng": -64.92625,
    "region": "St. Thomas — South Shore",
    "type": "site"
  },
  {
    "name": "Hull Bay",
    "lat": 18.37123,
    "lng": -64.95166,
    "region": "St. Thomas — North Shore",
    "type": "site"
  },
  {
    "name": "Peterborg",
    "lat": 18.37781,
    "lng": -64.93701,
    "region": "St. Thomas — North Shore",
    "type": "site"
  },
  {
    "name": "Stumpy",
    "lat": 18.36427,
    "lng": -65.0075,
    "region": "St. Thomas — North Shore",
    "type": "site"
  },
  {
    "name": "Coki",
    "lat": 18.35018,
    "lng": -64.86515,
    "region": "St. Thomas — North Shore",
    "type": "site"
  },
  {
    "name": "Great St. James",
    "lat": 18.30546,
    "lng": -64.82767,
    "region": "East End Cays",
    "type": "site"
  },
  {
    "name": "Cow and Calf Rocks",
    "lat": 18.30423,
    "lng": -64.84668,
    "region": "East End Cays",
    "type": "site"
  },
  {
    "name": "Stragglers",
    "lat": 18.30304,
    "lng": -64.83612,
    "region": "East End Cays",
    "type": "site"
  },
  {
    "name": "Lovango",
    "lat": 18.36064,
    "lng": -64.80386,
    "region": "East End Cays",
    "type": "site"
  },
  {
    "name": "Henry's Reef",
    "lat": 18.36481,
    "lng": -64.80308,
    "region": "East End Cays",
    "type": "site"
  },
  {
    "name": "Murder Rock",
    "lat": 18.35965,
    "lng": -64.80264,
    "region": "East End Cays",
    "type": "site"
  },
  {
    "name": "Waterlemon Cay",
    "lat": 18.36672,
    "lng": -64.72319,
    "region": "St. John",
    "type": "site"
  },
  {
    "name": "Leinster",
    "lat": 18.36506,
    "lng": -64.72429,
    "region": "St. John",
    "type": "site"
  },
  {
    "name": "Mary Creek",
    "lat": 18.36451,
    "lng": -64.73123,
    "region": "St. John",
    "type": "site"
  },
  {
    "name": "Yawzi Point",
    "lat": 18.31223,
    "lng": -64.76566,
    "region": "St. John",
    "type": "site"
  }
];
